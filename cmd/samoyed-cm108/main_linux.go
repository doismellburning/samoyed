...
func main() {
	...
	for i := range things {
		...
		fmt.Printf("%2s  %04x %04x  %-*s %-*s %-*s %-*s %s %-*s\n",
			...
			len(things[i].DevnodeSound), things[i].DevnodeSound,
			len(things[0].Plughw)/5, things[i].Plughw,
			len(things[0].Plughw2)/4, things[i].Plughw2,
			things[i].DevnodeHidraw, len(things[i].DevnodeUSB), things[i].DevnodeUSB)
