...
type agwpe_s struct {
	portx            int /* 0 for first, 1 for second, etc. */
	port_hi_reserved int
	kind_lo          int /* message type */
	kind_hi          int
	call_from        string
	call_to          string
	data_len         int /* Number of data bytes following. */
	user_reserved    int
}
...
func main() {
	...
	*/
	...
	var server_sock = connect_to_server(hostname, port)
	...
	var err = SOCK_SEND(server_sock, (&mon_cmd), sizeof(mon_cmd))
	...
	for {
		var n = SOCK_RECV(server_sock, (&mon_cmd), sizeof(mon_cmd))
		...
		if mon_cmd.kind_lo == 'K' {
			...
			*/
			...
			channel = mon_cmd.portx
			memset(&alevel, 0, sizeof(alevel))
			// FIXME KG pp = ax25_from_frame ((unsigned char *)(data+1), mon_cmd.data_len-1, alevel);
			ax25_format_addrs(pp, result)
			// FIXME KG info_len = ax25_get_info (pp, (unsigned char **)(&pinfo));
			// FIXME KG pinfo[info_len] = '\0';
			...
			*/
			...
			if *pinfo == 't' {
				...
				*/
				...
				xmit_raw.hdr.portx = channel
				xmit_raw.hdr.kind_lo = 'K'
				xmit_raw.hdr.data_len = 1 + ax25_pack(reply_pp, xmit_raw.frame)

				err = SOCK_SEND(server_sock, (&xmit_raw), sizeof(xmit_raw.hdr)+xmit_raw.hdr.data_len)
...
func connect_to_server(hostname string, port string) int {
	...
	int err;
	*/
	var ipaddr_str string /* text form of IP address */
	...
	num_hosts = 0
	for ai = ai_head; ai != nil; ai = ai.ai_next {
	...
	for n = 0; n < num_hosts; n++ {
		...
		var is = socket(ai.ai_family, ai.ai_socktype, ai.ai_protocol)
		if err != 0 {
			printf("Socket creation failed, err=%s", gai_strerror(err))
			// FIXME KG(void) close (is);
			is = -1
			continue
		}

		var err = connect(is, ai.ai_addr, ai.ai_addrlen)
		if err != 0 {
			// FIXME KG (void) close (is);
			is = -1
			continue
		...
		var flag = 1
		err = setsockopt(is, IPPROTO_TCP, TCP_NODELAY, (&flag), sizeof(flag))
		if err < 0 {
