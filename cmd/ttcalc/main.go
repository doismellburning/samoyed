...
type agwpe_s struct {
	portx            uint16 /* 0 for first, 1 for second, etc. */
	port_hi_reserved uint16
	kind_lo          uint16 /* message type */
	kind_hi          uint16
	call_from        [10]byte
	call_to          [10]byte
	data_len         uint32 /* Number of data bytes following. */
	user_reserved    uint32
}
...
func main() {
	...
	for {
		...
		if mon_cmd.kind_lo == 'K' {
			...
			var pp = C.ax25_from_frame((*C.uchar)(&data[1]), C.int(mon_cmd.data_len-1), alevel)
			...
			if *pinfo == 't' {
				...
				hdr.data_len = 1 + uint32(reply_len)
				...
				server_sock.Write(unsafe.Slice(&reply_bytes[0], hdr.data_len-1))
