...
func main() {
	...
	for j := range num_clients {
		...
		if unicode.IsDigit(rune(port[j][0])) {
			go client_thread_net(j, ch)
		} else {
			go client_thread_serial(j, ch)
		}
	}
...
func client_thread_net(my_index int, packetChan chan<- string) {
	...
	for {
		...
		if mon_cmd.DataKind == 'K' && (use_chan == -1 || byte(use_chan) == mon_cmd.Portx) {
			...
			var pp = C.ax25_from_frame(&dataUChar[0], C.int(mon_cmd.DataLen-1), alevel)
...
func client_thread_serial(my_index int, packetChan chan<- string) {
	var fd = C.serial_port_open(C.CString(port[my_index]), 9600)
