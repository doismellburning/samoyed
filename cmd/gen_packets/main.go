...
EAS for Emergency Alert System (EAS) Specific Area Message Encoding (SAME).`)
	...
	*/
	...
	C.morse_init(&modem, C.int(*amplitude/2))
	C.dtmf_init(&modem, C.int(*amplitude/2))
...
func send_packet(str string) {
	if g_morse_wpm > 0 {
		...
		C.morse_send(0, C.CString(str), C.int(g_morse_wpm), 100, 100)
		...
		C.eas_send(0, pinfo, repeat, 500, 500)
		C.ax25_delete(pp)
	} else {
		var pp = C.ax25_from_text(C.CString(str), 1)
		if pp == nil {
		...
		for c := C.int(0); c < modem.adev[0].num_channels; c++ {
			...
			C.layer2_preamble_postamble(c, 32, 0, &modem)
			C.layer2_send_frame(c, pp, 0, &modem)
			C.layer2_preamble_postamble(c, 2, 1, &modem)
		}
		C.ax25_delete(pp)
	}
}
