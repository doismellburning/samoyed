...
func TestProcessFromTNCFollowsASession(t *testing.T) {
	...
	// The TNC says how much is still waiting to go to it.
	process_from_tnc(fromTNC('Y', 0, testMyCall, testTheirCall, "3"))

	s.mu.Lock()
	assert.Equal(t, maybe.Just(3), s.txQueueLen)
	s.mu.Unlock()
...
func TestProcessFromTNCPortInformation(t *testing.T) {
	...
	process_from_tnc(fromTNC('G', 0, Callsign{}, Callsign{}, "2;Port1 first;Port2 second;"))

	assert.Equal(t, []tncFrame{{kind: 'X', channel: 0, callFrom: testMyCall, callTo: Callsign{}, data: ""}}, tnc.frames(t))
}
