...
* A value that might not be there is `maybe.Maybe[T]` (`internal/maybe`, modelled on Haskell's `Maybe`), not the `G_UNKNOWN` sentinel inherited from Dire Wolf. Its zero value is Nothing, so a new struct needs no "clear everything to unknown" block, and `maybe.Fmap`/`LiftA2`/`Bind` keep the absence from leaking into arithmetic. `G_UNKNOWN` survives where the conversion hasn't reached yet (see issue #619); `decode_aprs.go`'s `unlessUnknown`/`orUnknown` bridge the boundary
