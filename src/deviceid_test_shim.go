...
func deviceid_test_main(t *testing.T) {
	...
	comment_out, device = d.deviceid_decode_mice(">Comment")
	...
	comment_out, device = d.deviceid_decode_mice("]\"4V}=")
	assert.Equal(t, "\"4V}", comment_out)
	assert.Equal(t, "Kenwood TM-D710", device)

	// Modern MIC-E.

	comment_out, device = d.deviceid_decode_mice("`Comment_\"")
	assert.Equal(t, "Comment", comment_out)
	...
	comment_out, device = d.deviceid_decode_mice("'Comment|3")
	assert.Equal(t, "Comment", comment_out)
	...
	comment_out, device = d.deviceid_decode_mice("")
	assert.Empty(t, comment_out)
	assert.Equal(t, "UNKNOWN vendor/model", device)

	// Tocall

	device = d.deviceid_decode_dest("APDW18")
	assert.Equal(t, "WB2OSZ DireWolf", device)

	device = d.deviceid_decode_dest("APD123")
	assert.Equal(t, "Open Source aprsd", device)

	// null for Vendor.
	device = d.deviceid_decode_dest("APAX")
	assert.Equal(t, "AFilterX", device)

	device = d.deviceid_decode_dest("APA123")
	assert.Equal(t, "UNKNOWN vendor/model", device)
}
