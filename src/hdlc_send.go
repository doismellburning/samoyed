...
func layer2_send_frame(channel C.int, pp C.packet_t, bad_fcs C.int, audio_config_p *C.struct_audio_s) C.int {
	...
	if audio_config_p.achan[channel].layer2_xmit == C.LAYER2_IL2P { //nolint:staticcheck
		...
		var n = C.fx25_send_frame(channel, &fbuf[0], flen, audio_config_p.achan[channel].fx25_strength, 0)
		if n > 0 {
			return (n)
		}
		text_color_set(DW_COLOR_ERROR)
...
func eas_send(channel C.int, _str *C.uchar, repeat C.int, txdelay C.int, txtail C.int) C.int {
	...
	C.gen_tone_put_quiet_ms(channel, txdelay)
	...
	for r := C.int(0); r < repeat; r++ {
		...
		if r < repeat-1 {
			C.gen_tone_put_quiet_ms(channel, gap)
			gaps_sent++
		}
	}

	C.gen_tone_put_quiet_ms(channel, txtail)
