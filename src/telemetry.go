...
// FIXME KG #define MAGIC1  0x5a1111a5			/* For checking storage allocation problems. */
// FIXME KG #define MAGIC2  0x5a2222a5

// FIXME KG #define C_A 0					/* Scaling coefficient positions. */
// FIXME KG #define C_B 1
// FIXME KG #define C_C 2

// FIXME KG static 	struct t_metadata_s * md_list_head = nil;
...
func t_get_metadata(station *C.char) *C.struct_t_metadata_s {
...
func telemetry_data_original(station *C.char, info *C.char, quiet C.int, output *C.char, outputsize C.size_t, comment *C.char, commentsize C.size_t) {
	/* FIXME KG
	int n;
	int seq;
	char stemp[256];
	...
	*/
	...
	seq = atoi(p)
	n = 0
	// FIXME KG for ((p = strsep(&next,",")) != nil) {
	for { // FIXME KG
		if n < T_NUM_ANALOG {
			if strlen(p) > 0 {
				araw[n] = atof(p)
...
// FIXME KG #define isdigit91(c) ((c) >= B91_MIN && (c) <= B91_MAX)
...
func telemetry_data_base91(station *C.char, cdata *C.char, output *C.char, outputsize C.size_t) {
	/* FIXME KG
	int n;
	int seq;
	char *p;
...
func telemetry_name_message(station *C.char, msg *C.char) {
	/* FIXME KG
	int n;
	char stemp[256];
	char *next;
...
func telemetry_unit_label_message(station *C.char, msg *C.char) {
	/* FIXME KG
	int n;
	char stemp[256];
	char *next;
...
func telemetry_coefficents_message(station *C.char, msg *C.char, quiet C.int) {
	/* FIXME KG
	int n;
	char stemp[256];
	char *next;
...
func telemetry_bit_sense_message(station *C.char, msg *C.char, quiet C.int) {
	/* FIXME KG
	int n;
	*/
...
func fval_to_str(float x, int ndp, char str[VAL_STR_SIZE]) {
	if x == G_UNKNOWN {
...
func ival_to_str(int x, char str[VAL_STR_SIZE]) {
	if x == G_UNKNOWN {
		strlcpy(str, "?", VAL_STR_SIZE)
	} else {
...
func t_data_process(pm *C.struct_t_metadata_s, seq C.int, araw [T_NUM_ANALOG]C.float, ndp [T_NUM_ANALOG]C.int, draw [T_NUM_DIGITAL]C.int, output *C.char, outputsize C.size_t) {
	/* FIXME KG
	int n;
	...
	for n = 0; n < T_NUM_ANALOG; n++ {
		...
		if araw[n] != G_UNKNOWN {
			...
			if araw[n] == G_UNKNOWN {
				...
				/* FIXME KG
				   var z = pm.coeff_ndp[n][C_A] == 0 ? 0 : pm.coeff_ndp[n][C_A] + ndp[n] + ndp[n];
