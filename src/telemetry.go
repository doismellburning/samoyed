...
type t_metadata_s struct {
	...
	sense [T_NUM_DIGITAL]int /* Polarity for digital channels. */

	magic2 int
}

/* TODO KG Since Go doesn't have `static` these can't be file scoped, so let's use the ones in ax25_link.go and see what happens!
...
var md_list_head *C.struct_t_metadata_s
...
func t_get_metadata(station *C.char) *C.struct_t_metadata_s {
	...
	*/
	...
	for p := md_list_head; p != nil; p = p.pnext {
		if C.strcmp(C.CString(station), p.station) == 0 {

			if p.magic1 != MAGIC1 || p.magic2 != MAGIC2 {
				text_color_set(DW_COLOR_ERROR)
				dw_printf("Internal error: REPORT THIS! Bad magic values\n")
			}
			return (p)
		}
	}

	var p = new(C.struct_t_metadata_s)
	/* FIXME KG
	...
	*/
	...
	C.strlcpy(p.station, C.CString(station), sizeof(p.station))
...
func fval_to_str(x C.float, ndp C.int, str [VAL_STR_SIZE]C.char) {
	if x == G_UNKNOWN {
...
func ival_to_str(x C.int, str [VAL_STR_SIZE]C.char) {
	if x == G_UNKNOWN {
		strlcpy(str, "?", VAL_STR_SIZE)
	} else {
