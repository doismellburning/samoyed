...
func (b *axudpBridge) broadcastKISS(ax25frame []byte) {
	// Pre-check: minimum KISS overhead is 3 bytes (FEND + type byte + FEND).
	// If the raw AX.25 frame alone fills the limit there is no room for any
	// KISS framing, so skip the expensive encoding step entirely.
	if len(ax25frame) >= MAX_KISS_LEN {
		fmt.Fprintf(os.Stderr, "samoyed-axudp: dropping oversized AX.25 frame (%d bytes >= %d), too large to KISS-encode\n", len(ax25frame), MAX_KISS_LEN)
		return
	}
