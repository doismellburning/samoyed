...
func symbols_test_main(t *testing.T) {
	...
	*/
	...
	symbols_get_description('/', ' ', &description[0], len(description))
...
func symbols_from_dest_or_src(dti rune, src string, dest string, symtab *C.char, symbol *C.char) {
	C.symbols_from_dest_or_src(C.char(dti), C.CString(src), C.CString(dest), symtab, symbol)
}

func symbols_into_dest(symtab rune, symbol rune, dest *C.char) int {
	return int(C.symbols_into_dest(C.char(symtab), C.char(symbol), dest))
}

func symbols_get_description(symtab rune, symbol rune, description *C.char, desc_size int) {
	C.symbols_get_description(C.char(symtab), C.char(symbol), description, C.size_t(desc_size))
}

func symbols_code_from_description(overlay rune, description string, symtab *C.char, symbol *C.char) int {
	return int(C.symbols_code_from_description(C.char(overlay), C.CString(description), symtab, symbol))
}
