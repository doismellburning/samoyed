...
func il2p_encode_frame(pp *packet_t, max_fec C.int, iout *C.uchar) C.int {
	...
	var hdr, e = il2p_type_1_header(pp, int(max_fec))

	if e >= 0 {
		var _scrambled = il2p_scramble_block(hdr)
		...
		var _iout, k = il2p_encode_payload(pinfo, max_fec, (*C.uchar)(unsafe.Add(unsafe.Pointer(iout), out_len)))
		C.memcpy(unsafe.Pointer(iout), C.CBytes(_iout), C.size_t(len(_iout)))
		if k > 0 {
			out_len += k
		...
		hdr, e = il2p_type_0_header(pp, int(max_fec))
		if e > 0 {
...
func il2p_decode_header_payload(uhdr []byte, epayload []byte, symbols_corrected *int) *packet_t {
	...
	if hdr_type == 1 {
		...
		if payload_len > 0 {
			...
			var extracted, e = il2p_decode_payload(epayload, max_fec, symbols_corrected)
