 ...
 *		However the "new" symbols, which give new meanings to
...
func symbols_init() {
	...
	var GOOD_LINE = func(x string) bool {
		return (len(x) > 6 &&
			(x[COL1_OVERLAY] == '/' || x[COL1_OVERLAY] == '\\' || unicode.IsUpper(rune(x[COL1_OVERLAY])) || unicode.IsDigit(rune(x[COL1_OVERLAY]))) &&
			x[COL2_SYMBOL] >= '!' &&
			x[COL2_SYMBOL] <= '~' &&
			x[COL3_SP] == ' ' &&
