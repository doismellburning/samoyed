...
// Mostly fsk_demod_state.h from Dire Wolf
...
func GenericDemodConfig() *DemodConfig {
	var c = new(DemodConfig)
...
func pll_dcd_signal_transition2 (demodConfig *DemodConfig, D *C.struct_demodulator_state_s, slice C.int, dpll_phase C.int) {
	if (dpll_phase > - demodConfig.DCD_GOOD_WIDTH * 1024 * 1024 && dpll_phase < demodConfig.DCD_GOOD_WIDTH * 1024 * 1024) {
...
func pll_dcd_each_symbol2 (demodConfig *DemodConfig, D *C.struct_demodulator_state_s, channel C.int, subchan C.int, slice C.int) {
	...
	var goodBits = bits.OnesCount(D.slicer[slice].good_hist)
	var badBits = bits.OnesCount(D.slicer[slice].bad_hist)
	if good - bad >= 2 {
		D.slicer[slice].score |= 1
	}

	var s = bits.OnesCount(D.slicer[slice].score);
	if (s >= demodConfig.DCD_THRESH_ON) {
	  if (D.slicer[slice].data_detect == 0) {
	    D.slicer[slice].data_detect = 1;
	    dcd_change (channel, subchan, slice, D.slicer[slice].data_detect);
	  }
	} else if (s <= demodConfig.DCD_THRESH_OFF) {
	  if (D.slicer[slice].data_detect != 0) {
