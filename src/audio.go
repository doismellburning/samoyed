...
type adev_s struct {
	...
	// PortAudio buffers (typed for Read/Write)
	paInputBuf16  []int16
	paOutputBuf16 []int16
	paInputBuf8   []uint8
	paOutputBuf8  []uint8

	// Byte-level buffers (maintains existing interface)
...
// Track if PortAudio has been initialized
...
func findPortAudioDevice(name string, forInput bool) *portaudio.DeviceInfo {
	...
	// Try substring match (for ALSA hw:X,Y style names)
	for _, dev := range devices {
...
func audio_open(pa *audio_s) C.int {
	...
	for a := C.int(0); a < MAX_ADEVS; a++ {
		if pa.adev[a].defined != 0 {
			...
			bufSizeInBytes := calcbufsize(pa.adev[a].samples_per_sec, pa.adev[a].num_channels, pa.adev[a].bits_per_sample)
			framesPerBuffer := bufSizeInBytes / adev[a].bytesPerFrame
			...
			case AUDIO_IN_TYPE_SOUNDCARD:
				inputDev := findPortAudioDevice(audio_in_name, true)
				if inputDev == nil {
					text_color_set(DW_COLOR_ERROR)
				...
				// Allocate PortAudio typed buffers
				if pa.adev[a].bits_per_sample == 16 {
					adev[a].paInputBuf16 = make([]int16, framesPerBuffer*pa.adev[a].num_channels)
				} else {
					adev[a].paInputBuf8 = make([]uint8, framesPerBuffer*pa.adev[a].num_channels)
				}

				// Open input stream
				var err error
				if pa.adev[a].bits_per_sample == 16 {
					adev[a].inputStream, err = portaudio.OpenStream(inputParams, adev[a].paInputBuf16)
				} else {
					adev[a].inputStream, err = portaudio.OpenStream(inputParams, adev[a].paInputBuf8)
				}
				if err != nil {
					text_color_set(DW_COLOR_ERROR)
			...
			outputDev := findPortAudioDevice(audio_out_name, false)
			if outputDev == nil {
				text_color_set(DW_COLOR_ERROR)
			...
			// Allocate PortAudio typed buffers for output
			if pa.adev[a].bits_per_sample == 16 {
				adev[a].paOutputBuf16 = make([]int16, framesPerBuffer*pa.adev[a].num_channels)
			} else {
				adev[a].paOutputBuf8 = make([]uint8, framesPerBuffer*pa.adev[a].num_channels)
			}

			// Open output stream
			var err error
			if pa.adev[a].bits_per_sample == 16 {
				adev[a].outputStream, err = portaudio.OpenStream(outputParams, adev[a].paOutputBuf16)
			} else {
				adev[a].outputStream, err = portaudio.OpenStream(outputParams, adev[a].paOutputBuf8)
			}
			if err != nil {
				text_color_set(DW_COLOR_ERROR)
...
func audio_get_real(a C.int) C.int {
	...
	case AUDIO_IN_TYPE_SOUNDCARD:
		...
		for adev[a].inbufNext >= adev[a].inbufLen {
			...
			Assert(adev[a].inputStream != nil)

			// Read from PortAudio stream
			var err error
			if adev[a].bitsPerSample == 16 {
				err = adev[a].inputStream.Read()
			} else {
				err = adev[a].inputStream.Read()
			}
			...
			if err != nil {
				text_color_set(DW_COLOR_ERROR)
				dw_printf("Audio input device %d error: %v\n", a, err)

				// Handle input overflow (similar to ALSA overrun)
				if strings.Contains(err.Error(), "overflow") || strings.Contains(err.Error(), "Input overflowed") {
					...
					dw_printf("This might be a temporary condition so we will attempt to recover.\n")
				}
			...
			// Convert typed buffer to byte buffer
			if adev[a].bitsPerSample == 16 {
				n := len(adev[a].paInputBuf16)
				// Ensure inbuf is large enough
				neededBytes := n * 2
				if len(adev[a].inbuf) < neededBytes {
					adev[a].inbuf = make([]byte, neededBytes)
				}
				for i := 0; i < n; i++ {
					binary.LittleEndian.PutUint16(adev[a].inbuf[i*2:], uint16(adev[a].paInputBuf16[i]))
				}
				adev[a].inbufLen = neededBytes
			} else {
				n := len(adev[a].paInputBuf8)
				if len(adev[a].inbuf) < n {
					adev[a].inbuf = make([]byte, n)
				}
				copy(adev[a].inbuf, adev[a].paInputBuf8)
				adev[a].inbufLen = n
			}
			adev[a].inbufNext = 0
	...
	case AUDIO_IN_TYPE_SDR_UDP:
		...
		for adev[a].inbufNext >= adev[a].inbufLen {
			...
			buf := make([]byte, adev[a].inbufSizeInBytes)
			n, _, readErr := adev[a].udp_sock.ReadFromUDP(buf)
	...
	if adev[a].inbufNext < adev[a].inbufLen {
		n = C.int(adev[a].inbuf[adev[a].inbufNext])
		adev[a].inbufNext++
...
func audio_put_real(a C.int, c C.int) C.int {
	...
	adev[a].outbuf[adev[a].outbufLen] = byte(c)
	adev[a].outbufLen++
...
func audio_flush_real(a C.int) C.int {
	...
	Assert(adev[a].outputStream != nil)

	// Make sure the output stream is started
	// PortAudio streams need to be started before writing
	info := adev[a].outputStream.Info()
	if info.SampleRate == 0 {
		// Stream might not be properly initialized
		text_color_set(DW_COLOR_ERROR)
		dw_printf("Audio output stream not properly initialized.\n")
		adev[a].outbufLen = 0
		return -1
	}

	// Start the stream if not already running
	err := adev[a].outputStream.Start()
	if err != nil && !strings.Contains(err.Error(), "already") {
		// Ignore "stream already active" errors
		text_color_set(DW_COLOR_ERROR)
		dw_printf("Audio output start error: %v\n", err)
	}

	// Convert byte buffer to typed buffer
	numFrames := adev[a].outbufLen / adev[a].bytesPerFrame
	if numFrames == 0 {
		adev[a].outbufLen = 0
		return 0
	}
	...
	for retries > 0 {
		if adev[a].bitsPerSample == 16 {
			...
			numSamples := numFrames * adev[a].numChannels
			if len(adev[a].paOutputBuf16) < numSamples {
				adev[a].paOutputBuf16 = make([]int16, numSamples)
			}
			for i := 0; i < numSamples; i++ {
				if i*2+1 < adev[a].outbufLen {
					adev[a].paOutputBuf16[i] = int16(binary.LittleEndian.Uint16(adev[a].outbuf[i*2:]))
				}
			}
			err = adev[a].outputStream.Write()
		} else {
			// 8-bit audio
			numSamples := numFrames * adev[a].numChannels
			if len(adev[a].paOutputBuf8) < numSamples {
				adev[a].paOutputBuf8 = make([]uint8, numSamples)
			}
			copy(adev[a].paOutputBuf8, adev[a].outbuf[:numSamples])
			err = adev[a].outputStream.Write()
		}

		if err != nil {
			text_color_set(DW_COLOR_ERROR)
			dw_printf("Audio write error: %v\n", err)

			if strings.Contains(err.Error(), "underflow") || strings.Contains(err.Error(), "Output underflowed") {
				dw_printf("Audio output data underrun.\n")
				// This is recoverable, just continue
			}
			retries--
			SLEEP_MS(10)
			continue
...
func audio_wait(a C.int) {
	...
	if adev[a].outputStream != nil {
		// Stop the stream, which waits for pending buffers to complete
		// This is equivalent to snd_pcm_drain in ALSA
		err := adev[a].outputStream.Stop()
		if err != nil && !strings.Contains(err.Error(), "stopped") {
			text_color_set(DW_COLOR_ERROR)
			dw_printf("Audio wait/stop error: %v\n", err)
		}
	}
