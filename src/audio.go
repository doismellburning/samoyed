...
func (rb *audioRingBuffer) write(data []byte) bool {
	...
	for _, b := range data {
		if rb.count >= rb.size {
			// Buffer full - drop oldest data (overwrite)
			rb.readPos = (rb.readPos + 1) % rb.size
			rb.count--
			overflow = true
		}
		rb.buf[rb.writePos] = b
...
// read retrieves one byte from the ring buffer. Blocks if empty.
// Returns the byte and true, or 0 and false if the buffer is closed.
func (rb *audioRingBuffer) read() (byte, bool) {
	rb.mu.Lock()
	defer rb.mu.Unlock()
...
// readNonBlocking retrieves one byte without blocking.
// Returns the byte and true, or 0 and false if empty.
func (rb *audioRingBuffer) readNonBlocking() (byte, bool) {
	rb.mu.Lock()
	defer rb.mu.Unlock()
...
type adev_s struct {
	...
	// Ring buffer for output audio (read by callback)
	outputRingBuf *audioRingBuffer
...
func audio_open(pa *audio_s) C.int {
	...
	for a := C.int(0); a < MAX_ADEVS; a++ {
		if pa.adev[a].defined != 0 {
			...
			case AUDIO_IN_TYPE_SOUNDCARD:
				...
				if pa.adev[a].bits_per_sample == 16 {
					adev[a].inputStream, err = portaudio.OpenStream(
						inputParams,
						func(in []int16) {
							// Convert int16 samples to bytes and write to ring buffer
							var bytes = make([]byte, len(in)*2)
							for i, sample := range in {
								binary.LittleEndian.PutUint16(bytes[i*2:], uint16(sample))
							}
							inRingBuf.write(bytes)
						},
					)
				} else {
					adev[a].inputStream, err = portaudio.OpenStream(
			...
			/*
			 * Output device with callback mode.
			 * The callback reads from a ring buffer and outputs silence if empty.
			 * This is more reliable for bursty packet radio transmissions.
			 */
			...
			// Create ring buffer for output audio data.
			// Size it to hold ~1 second of audio.
			var outRingBufSize = pa.adev[a].samples_per_sec * pa.adev[a].num_channels * pa.adev[a].bits_per_sample / 8
			adev[a].outputRingBuf = newAudioRingBuffer(outRingBufSize)
			...
			// The callback reads from the ring buffer and outputs silence if empty.
			// IMPORTANT: Capture the ring buffer pointer now, not in the closure,
			// to avoid the classic Go closure-over-loop-variable bug.
			var outRingBuf = adev[a].outputRingBuf
			var err error
			if pa.adev[a].bits_per_sample == 16 {
				adev[a].outputStream, err = portaudio.OpenStream(
					outputParams,
					func(out []int16) {
						// Read from ring buffer and convert bytes to int16
						for i := range out {
							var lo, okLo = outRingBuf.readNonBlocking()
							var hi, okHi = outRingBuf.readNonBlocking()
							if okLo && okHi {
				...
				adev[a].outputStream, err = portaudio.OpenStream(
					...
					func(out []uint8) {
						...
						for i := range out {
							var b, ok = outRingBuf.readNonBlocking()
							if ok {
								out[i] = b
							} else {
								out[i] = 128 // Silence for unsigned 8-bit
							}
						}
					},
				)
...
func audio_get_real(a C.int) C.int {
	...
	case AUDIO_IN_TYPE_SOUNDCARD:
		...
		// Read one byte from the ring buffer (blocks if empty)
		b, ok := adev[a].inputRingBuf.read()
		if !ok {
	...
	case AUDIO_IN_TYPE_SDR_UDP:
		...
		for adev[a].inbufNext >= adev[a].inbufLen {
			...
			var buf = make([]byte, adev[a].inbufSizeInBytes)
...
func audio_flush_real(a C.int) C.int {
	...
	Assert(adev[a].outputRingBuf != nil)

	// Write the audio data to the output ring buffer.
	// The PortAudio callback will read from this buffer.
...
func audio_wait(a C.int) {
	...
	// Note: We keep the output stream running continuously rather than
	// stopping it after each transmission. This is more reliable with
	// PortAudio's blocking write mode.
