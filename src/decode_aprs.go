...
type compressed_position_t struct {
	...
	symbol_code C.char
	c           C.char /* Course/speed or altitude. */
	s           C.char
...
func decode_aprs(A *C.decode_aprs_t, pp C.packet_t, quiet C.int, third_party_src *C.char) {
	...
	if third_party_src == nil {
		C.memset(unsafe.Pointer(A), 0, C.sizeof_decode_aprs_t)
	}
	...
	for i := C.int(0); i < C.ax25_get_num_repeaters(pp); i++ {
		...
		if quiet == 0 {
			if "RELAY" == atemp || "WIDE" == atemp || "TRACE" == atemp {
				text_color_set(DW_COLOR_ERROR)
	...
	if pinfo[0] == '}' {
		...
		var pp_payload = C.ax25_from_text((*C.char)(C.CBytes(pinfo[1:])), 0)
		if pp_payload != nil {
			var payload_src = pinfo[1:]
			payload_src, _, _ = bytes.Cut(payload_src, []byte{'>'})
			A.g_has_thirdparty_header = 1
			decode_aprs(A, pp_payload, quiet, (*C.char)(C.CBytes(payload_src))) // 1 means used recursively
	...
	*/
	...
	if A.g_lat != G_UNKNOWN || A.g_lon != G_UNKNOWN {
		...
		if A.g_lat != G_UNKNOWN {
			...
			var deg = int(absll)
			var min = (absll - C.double(deg)) * 60.0
			s_lat = fmt.Sprintf("%c %02d%s%07.4f", news, deg, C.CH_DEGREE, min)
		} else {
		...
		if A.g_lon != G_UNKNOWN {
			...
			var deg = int(absll)
			var min = (absll - C.double(deg)) * 60.0
			s_lon = fmt.Sprintf("%c %03d%s%07.4f", news, deg, C.CH_DEGREE, min)
		} else {
...
func aprs_ll_pos(A *C.decode_aprs_t, info []byte) {
	...
	type aprs_ll_pos_s struct {
		dti     byte /* ! or = */
		pos     position_t
		comment [43]byte /* Start of comment could be data extension(s). */
	}
	var p aprs_ll_pos_s
	...
	type aprs_compressed_pos_s struct {
		...
		comment [40]byte /* No data extension allowed for compressed location. */
	}
...
func aprs_raw_nmea(A *C.decode_aprs_t, info []byte) {
	if bytes.HasPrefix(info, []byte("$GPRMC,")) ||
		...
		C.dwgpsnmea_gprmc((*C.char)(C.CBytes(info)), A.g_quiet, &(A.g_lat), &(A.g_lon), &speed_knots, &(A.g_course))
		...
		C.dwgpsnmea_gpgga((*C.char)(C.CBytes(info)), A.g_quiet, &(A.g_lat), &(A.g_lon), &alt_meters, &num_sat)
...
func aprs_mic_e(A *C.decode_aprs_t, pp C.packet_t, info []byte) {
	...
	A.g_symbol_table = C.char(p.sym_table_id)
	A.g_symbol_code = C.char(p.symbol_code)
	...
	var n = int((p.speed_course[0]-28)*10) + int((p.speed_course[1]-28)/10)
	if n >= 800 {
		n -= 800
	}
	...
	n = int((p.speed_course[1]-28)%10)*100 + int(p.speed_course[2]-28)
	if n >= 400 {
		n -= 400
	}
	...
	if C.strlen(&trimmed[0]) >= 4 &&
		isdigit91(rune(trimmed[0])) &&
		isdigit91(rune(trimmed[1])) &&
		isdigit91(rune(trimmed[2])) &&
		trimmed[3] == '}' {
...
func aprs_message(A *C.decode_aprs_t, info []byte, quiet bool) {
	...
	type aprs_message_s struct {
		...
		addressee [9]byte
		colon     byte                  /* : */
		message   [256 - 1 - 9 - 1]byte /* Officially up to 67 characters for message text. */
	...
	*/
	...
	if len(addressee) >= 3 && bytes.HasPrefix(addressee, []byte("BLN")) {
		...
		A.g_message_subtype = C.message_subtype_nws
		C.strcpy(&A.g_comment[0], (*C.char)(C.CBytes(p.message[:])))
	} else if bytes.HasPrefix(p.message[:], []byte("PARM.")) {
		...
		telemetry_name_message(string(addressee), string(p.message[5:]))
	} else if bytes.HasPrefix(p.message[:], []byte("UNIT.")) {
		...
		telemetry_unit_label_message(string(addressee), string(p.message[5:]))
	} else if bytes.HasPrefix(p.message[:], []byte("EQNS.")) {
		...
		telemetry_coefficents_message(string(addressee), string(p.message[5:]), bool2Cint(quiet))
	} else if bytes.HasPrefix(p.message[:], []byte("BITS.")) {
		...
		telemetry_bit_sense_message(string(addressee), string(p.message[5:]), bool2Cint(quiet))
	} else if p.message[0] == '?' {
		...
		if found {
			...
			if C.strlen(&ack[0]) > 0 {
				...
				C.strcpy(&A.g_data_type_desc[0], C.CString(fmt.Sprintf("APRS Message, number \"%s\", from \"%s\" to \"%s\"", A.g_message_number, A.g_src, addressee)))
			}
		} else {
			// No message number.
			C.strcpy(&A.g_data_type_desc[0], C.CString(fmt.Sprintf("APRS Message, with no number, from \"%s\" to \"%s\"", A.g_src, addressee)))
		}
...
func aprs_status_report(A *C.decode_aprs_t, info []byte) {
	...
	if strlen(A.g_comment) >= 3 {
		var hp = A.g_comment + strlen(A.g_comment) - 3
...
func aprs_positionless_weather_report(A *C.decode_aprs_t, info []byte) {
	...
	// FIXME KG p = (struct aprs_positionless_weather_s *)info;
...
func getwdata(wpp **C.char, ch C.char, dlen C.int, val *C.float) C.int {
	/* FIXME KG
	...
	*val = G_UNKNOWN
	...
	for i := 1; i <= dlen; i++ {
		if !unicode.IsDigit((*wpp)[i]) && (*wpp)[i] != '.' && (*wpp)[i] != '-' {
			return (0)
		}
	}
...
func aprs_ultimeter(A *C.decode_aprs_t, info []byte) {
	...
	*/
	...
	if *info == '$' {
		var n = sscanf(info+5, "%4hx%4hx%4hx%4hx%4hx%4hx%4hx%4hx%4hx%4hx%4hx%4hx%4hx",
			&h_windpeak,
			&h_wdir,
			&h_otemp,
			&h_totrain,
...
func get_latitude_8(p *C.char, quiet C.int) C.double {
	type lat_s struct {
		deg  [2]byte
		minn [2]byte
		dot  byte
	...
	// FIXME KG plat = (lat_s *)p;

	if unicode.IsDigit(plat.deg[0]) {
	...
	if unicode.IsDigit(plat.minn[1]) {
	...
	if unicode.IsDigit(plat.hmin[0]) {
	...
	if unicode.IsDigit(plat.hmin[1]) {
...
func get_longitude_9(p *C.char, quiet C.int) C.double {
	type lat_s struct {
		deg  [3]byte
		minn [2]byte
		dot  byte
	...
	if unicode.IsDigit(plon.minn[1]) {
	...
	if unicode.IsDigit(plon.hmin[0]) {
	...
	if unicode.IsDigit(plon.hmin[1]) {
...
func get_timestamp(A *C.decode_aprs_t, p []byte) time.Time {
	type dhm_s struct {
		day     [2]byte
		hours   [2]byte
		minutes [2]byte
	...
	if !(unicode.IsDigit(p[0]) && unicode.IsDigit(p[1]) && unicode.IsDigit(p[2]) && unicode.IsDigit(p[3]) && unicode.IsDigit(p[4]) && unicode.IsDigit(p[5]) &&
		(p[6] == 'z' || p[6] == '/' || p[6] == 'h')) {
		text_color_set(DW_COLOR_ERROR)
	...
	*/
	...
	if pdhm.tic == 'z' || pdhm.tic == '/' { /* Wrong! */
		var j = (pdhm.day[0]-'0')*10 + pdhm.day[1] - '0'
		//text_color_set(DW_COLOR_DECODED);
		...
		var j = (phms.hours[0]-'0')*10 + phms.hours[1] - '0'
		//text_color_set(DW_COLOR_DECODED);
		//dw_printf("Changing hours from %d to %d\n", ptm.tm_hour, j);
		ptm.tm_hour = j
...
func get_maidenhead(A *C.decode_aprs_t, p *C.char) C.int {
	...
	if toupper(p[0]) >= 'A' && toupper(p[0]) <= 'R' &&
		...
		unicode.IsDigit(p[2]) && unicode.IsDigit(p[3]) {
...
/* FIXME KG
func substr_se (char *dest, const char *src, int start, int endp1)
{
	int len = endp1 - start;
...
// FIXME KG #define sign(x) (((x)>=0)?1:(-1))

func process_comment(A *C.decode_aprs_t, pstart []byte) {
	// FIXME KG static int first_time = 1;
	// FIXME KG static regex_t std_freq_re;	/* Frequency in standard format. */
	// FIXME KG static regex_t std_tone_re;	/* Tone in standard format. */
	// FIXME KG static regex_t std_toff_re;	/* Explicitly no tone. */
	// FIXME KG static regex_t std_dcs_re;	/* Digital codes squelch in standard format. */
	// FIXME KG static regex_t std_offset_re;	/* Xmit freq offset in standard format. */
	// FIXME KG static regex_t std_range_re;	/* Range in standard format. */

	// FIXME KG static regex_t dao_re;		/* DAO */
	// FIXME KG static regex_t alt_re;		/* /A= altitude */

	// FIXME KG static regex_t bad_freq_re;	/* Likely frequency, not standard format */
	// FIXME KG static regex_t bad_tone_re;	/* Likely tone, not standard format */

	// FIXME KG static regex_t base91_tel_re;	/* Base 91 compressed telemetry data. */
	...
	*/
	...
	if clen > 0 {
		memcpy(A.g_comment, pstart, clen)
		A.g_comment[clen] = 0
	} else {
		A.g_comment[0] = 0
	}
	...
	if regexec(&std_freq_re, A.g_comment, MAXMATCH, match, 0) == 0 {
		var sftemp [30]C.char
		var smtemp [10]C.char
	...
	for keep_going {
		...
		if regexec(&std_tone_re, A.g_comment, MAXMATCH, match, 0) == 0 {
			...
			var sttemp [10]C.char /* includes leading letter */
			...
			// FIXME KG char sttemp[10];	/* three octal digits */

			substr_se(sttemp, A.g_comment, match[1].rm_so, match[1].rm_eo)

			A.g_dcs = strtoul(sttemp, nil, 8)
			...
			// FIXME KG char sttemp[10];	/* includes leading sign */
			...
			// FIXME KG char sttemp[10];	/* should be two digits */
			// FIXME KG char sutemp[10];	/* m for miles or k for km */
	...
	if regexec(&base91_tel_re, A.g_comment, MAXMATCH, match, 0) == 0 {
		...
		// FIXME KG char tdata[30];	/* Should be even number of 4 to 14 characters. */
	...
	if regexec(&dao_re, A.g_comment, MAXMATCH, match, 0) == 0 {
		...
		var d = A.g_comment[match[0].rm_so+1]
		var a = A.g_comment[match[0].rm_so+2]
		var o = A.g_comment[match[0].rm_so+3]
	...
	if A.g_tone == G_UNKNOWN && regexec(&bad_tone_re, A.g_comment, MAXMATCH, match, 0) == 0 {
		// FIXME KG char bad1[30];	/* original 99.9 or 999.9 format or one of 67 77 100 123 */
		// FIXME KG char bad2[30];	/* 99.9 or 999.9 format.  ".0" appended for special cases. */
		// FIXME KG char good[30];
		// FIXME KG int i;
