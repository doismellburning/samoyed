...
func decode_aprs_print(A *decode_aprs_t) {
	...
	if n > 0 {
		ax25_safe_print(C.GoBytes(unsafe.Pointer(&A.g_weather[0]), C.int(len(A.g_weather))), 0)
		dw_printf("\n")
	}

	if C.strlen(&A.g_telemetry[0]) > 0 {
		ax25_safe_print(C.GoBytes(unsafe.Pointer(&A.g_telemetry[0]), C.int(len(A.g_telemetry))), 0)
		dw_printf("\n")
	}
	...
	if n > 0 {
		ax25_safe_print(C.GoBytes(unsafe.Pointer(&A.g_comment[0]), C.int(len(A.g_comment))), 0)
		dw_printf("\n")
