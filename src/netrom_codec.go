...
const (
	...
	// netromNodesPerFrame is the conventional NET/ROM limit on entries in one
	// NODES broadcast; 6 + 11*21 = 237 bytes fits comfortably in an AX.25
	// information field and matches what other implementations expect.
	netromNodesPerFrame = 11
)
...
// netromNodesPayloads encodes entries as one or more NODES broadcast
// payloads, each carrying at most netromNodesPerFrame entries. A single
// payload for a large table would exceed what an AX.25 information field can
// hold and be truncated mid-entry.
//
// An empty entry list still produces one payload, so a node with no routes
// announces itself rather than going silent.
func netromNodesPayloads(srcAlias [netromAliasLen]byte, entries []netromNodesEntry) [][]byte {
	if len(entries) == 0 {
		return [][]byte{netromBuildRoutingBroadcast(srcAlias, nil)}
	}

	var payloads [][]byte
	for start := 0; start < len(entries); start += netromNodesPerFrame {
		var end = min(start+netromNodesPerFrame, len(entries))
		payloads = append(payloads, netromBuildRoutingBroadcast(srcAlias, entries[start:end]))
	}
