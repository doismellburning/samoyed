...
func get_access_to_gpio(path *C.char) {
	/* FIXME KG
	int i;
	char cmd[80];
	*/
	...
	var _, err = os.Stat(C.GoString(path))
	if err != nil {
...
func export_gpio(ch C.int, ot C.int, invert C.int, direction C.int) {
	...
	*/
	...
	var gpio_num C.int
	var gpio_name *C.char

	if direction > 0 {
		gpio_num = save_audio_config_p.achan[ch].octrl[ot].out_gpio_num
		gpio_name = &save_audio_config_p.achan[ch].octrl[ot].out_gpio_name[0]
	} else {
		gpio_num = save_audio_config_p.achan[ch].ictrl[ot].in_gpio_num
		gpio_name = &save_audio_config_p.achan[ch].ictrl[ot].in_gpio_name[0]
	}
	...
	var fd = open(gpio_export_path, O_WRONLY)
	if fd < 0 {
	...
	*/
	...
	num_files = scandir("/sys/class/gpio", &file_list, nil, alphasort)
...
func gpiod_probe(chip_dev_path *C.char, line_number C.int) C.int {
	// chip_dev_path must be complete device path such as /dev/gpiochip3

	// FIXME KG struct gpiod_chip *chip;
	...
	// FIXME KG struct gpiod_line *line;
...
// FIXME KG static HANDLE ptt_fd[MAX_RADIO_CHANS][NUM_OCTYPES];
/* Serial port handle or fd.  */
/* Could be the same for two channels */
/* if using both RTS and DTR. */
// FIXME KG static RIG *rig[MAX_RADIO_CHANS][NUM_OCTYPES];

// FIXME KG static char otnames[NUM_OCTYPES][8];
			...
			for ot := 0; ot < NUM_OCTYPES; ot++ {
				...
				if audio_config_p.achan[ch].octrl[ot].ptt_method == PTT_METHOD_SERIAL {
					...
					if strncasecmp(audio_config_p.achan[ch].octrl[ot].ptt_device, "COM", 3) == 0 {
						var n = atoi(audio_config_p.achan[ch].octrl[ot].ptt_device + 3)
				...
				if audio_config_p.achan[ch].octrl[ot].ptt_method == PTT_METHOD_HAMLIB {
					if ot == OCTYPE_PTT {
						...
						if audio_config_p.achan[ch].octrl[ot].ptt_model == -1 {
							// FIXME KG hamlib_port_t hport;	// http://hamlib.sourceforge.net/manuals/1.2.15/structhamlib__port__t.html
...
func ptt_set_real(ot C.int, channel C.int, ptt_signal C.int) {
	...
	if save_audio_config_p.achan[channel].octrl[ot].ptt_method == PTT_METHOD_SERIAL &&
		ptt_fd[channel][ot] != INVALID_HANDLE_VALUE {

		if save_audio_config_p.achan[channel].octrl[ot].ptt_line == PTT_LINE_RTS {

			if ptt {
				RTS_ON(ptt_fd[channel][ot])
			} else {
				RTS_OFF(ptt_fd[channel][ot])
			}
		} else if save_audio_config_p.achan[channel].octrl[ot].ptt_line == PTT_LINE_DTR {

			if ptt {
				DTR_ON(ptt_fd[channel][ot])
			} else {
				DTR_OFF(ptt_fd[channel][ot])
			}
		}
		...
		if save_audio_config_p.achan[channel].octrl[ot].ptt_line2 == PTT_LINE_RTS {
		...
		} else if save_audio_config_p.achan[channel].octrl[ot].ptt_line2 == PTT_LINE_DTR {
	...
	if save_audio_config_p.achan[channel].octrl[ot].ptt_method == PTT_METHOD_GPIO {
		...
		*/
		...
		snprintf(gpio_value_path, sizeof(gpio_value_path), "/sys/class/gpio/%s/value", save_audio_config_p.achan[channel].octrl[ot].out_gpio_name)

		var fd = open(gpio_value_path, O_WRONLY)
		if fd < 0 {
			var int e = errno
			text_color_set(DW_COLOR_ERROR)
	...
	if save_audio_config_p.achan[channel].octrl[ot].ptt_method == PTT_METHOD_GPIOD {
		...
		var rc = gpiod_ctxless_set_value(chip, line, ptt, false, "direwolf", nil, nil)
		if ptt_debug_level >= 1 {
			text_color_set(DW_COLOR_DEBUG)
	...
	if save_audio_config_p.achan[channel].octrl[ot].ptt_method == PTT_METHOD_LPT &&
		...
		lseek(ptt_fd[channel][ot], LPT_IO_ADDR, SEEK_SET)
		if read(ptt_fd[channel][ot], &lpt_data, 1) != 1 {
			var e = errno
			text_color_set(DW_COLOR_ERROR)
		...
		if ptt {
			lpt_data |= (1 << save_audio_config_p.achan[channel].octrl[ot].ptt_lpt_bit)
		} else {
			lpt_data &= ~(1 << save_audio_config_p.achan[channel].octrl[ot].ptt_lpt_bit)
		}

		lseek(ptt_fd[channel][ot], LPT_IO_ADDR, SEEK_SET)
		if write(ptt_fd[channel][ot], &lpt_data, 1) != 1 {
			var e = errno
			text_color_set(DW_COLOR_ERROR)
	...
	if save_audio_config_p.achan[channel].octrl[ot].ptt_method == PTT_METHOD_HAMLIB {
		...
		if rig[channel][ot] != nil {
			...
			var onoff = RIG_PTT_OFF
			if ptt {
				onoff = RIG_PTT_ON
			}
			var retcode = rig_set_ptt(rig[channel][ot], RIG_VFO_CURR, onoff)
...
func get_input_real(it C.int, channel C.int) C.int {
	...
	if save_audio_config_p.achan[channel].ictrl[it].method == PTT_METHOD_GPIO {
		// FIXME KG char gpio_value_path[80];

		snprintf(gpio_value_path, sizeof(gpio_value_path), "/sys/class/gpio/%s/value", save_audio_config_p.achan[channel].ictrl[it].in_gpio_name)

		get_access_to_gpio(gpio_value_path)

		var fd = open(gpio_value_path, O_RDONLY)
		if fd < 0 {
			var e = errno
			text_color_set(DW_COLOR_ERROR)
		...
		vtemp[1] = 0
		if atoi(vtemp) != save_audio_config_p.achan[channel].ictrl[it].invert {
			return 1
		} else {
			return 0
		}
